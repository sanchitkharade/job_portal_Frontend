...
const MultiInput=(props:any)=> {
  ...
  const options = data.filter((items)=>items.toLowerCase().includes(search.trim().toLowerCase())).map((item) => (
  ...
  return (
    <Combobox store={combobox} onOptionSubmit={handleValueSelect} withinPortal={false}>
      <Combobox.DropdownTarget>
        ...
        leftSection={
            <div className='text-bright-sun-400 p-1 bg-mine-shaft-900 rounded-full mr-3'><props.icon/></div>
        }>
           <Pill.Group>
            {value.length > 0 ? (
              <>
